...
class ObjectDetection:
    ...
    def load_model(self):
        ...
        """
        model = YOLO("best.pt")
        model.conf = 0.75  # set confidence threshold
        model.classes = classes  # set class labels
        return model
    ...
    def score_frame(self, frame):
        ...
        cord = results[0].boxes.xyxyn.tolist()
        percent = results[0].boxes.conf.tolist()
    ...
    def plot_boxes(self, results, frame):
        ...
        for i in range(n):
            row = cord[i]
            if row[3] >= 0.2 and confidence[i] > 0.75:
                x1, y1, x2, y2 = int(
